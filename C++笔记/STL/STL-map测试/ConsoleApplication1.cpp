...
void print_in_order(map<int,int> &m)
{
    printf("顺序打印:\n");
...
void print_reverse_order(map<int, int>& m)
{
    printf("逆序打印:\n");
    //按逆序打印噢
...
int main()
{
    ...
    m.insert(pair<int, int>(520, 521));

    print_in_order(m);
    print_reverse_order(m);
    //使用下标访问的时候阿,[]里面的值是key
   /* printf("使用下标访问的时候\n");*/
    ...
    {
        cout << "success";
        cout << endl << "find(2)" << it->first << "->" << it->second << endl;
    }
    else
    {
        cout << "fail";
